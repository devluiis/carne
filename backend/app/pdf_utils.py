...
def _get_single_parcela_receipt_flowable(parcela, cliente_info, carne_info, styles, logo_data, qrcode_image):
    ...
    slip_elements.append(Spacer(1, 0.01*cm)) # Espaçamento ainda mais reduzido
    ...
    recibo_pagador_content = [
        ...
        Spacer(1, 0.01*cm), # Espaçamento ainda mais reduzido
        Table([
    ...
    main_boleto_content = [
        ...
        Table([
            [Image(logo_data, width=0.8*cm, height=0.8*cm) if logo_data else "", # Logo menor (ainda mais reduzido)
        ...
        ], colWidths=[2.2*cm, 10.8*cm], # QR Code reduzido e espaço para texto aumentado
        style=TableStyle([
            ('VALIGN', (0,0), (-1,-1), 'TOP'),
            ('LEFTPADDING', (0,0), (-1,-1), 0),
            ('RIGHTPADDING', (0,0), (-1,-1), 0),
    ...
    main_receipt_table.setStyle(TableStyle([
        ...
        # Removendo GRID temporariamente para depuração, caso ele cause problemas de cálculo de altura
        # ('GRID', (0,0), (-1,-1), 0.25, HexColor('#D3D3D3')) 
    ]))
