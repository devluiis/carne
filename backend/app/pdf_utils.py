...
from app.config import DATABASE_URL  # Exemplo
import os
...
class PDF(FPDF):
    def header(self):
        if os.path.exists(str(LOGO_PATH)):
            # self.image(str(LOGO_PATH), 10, 8, 33)  # Para ativar logo real
            self.set_font('Helvetica', 'B', 10)
            self.cell(0, 10, '>>> LOGO TESTE OK <<<', 0, 1, 'L')
        else:
            self.set_font('Helvetica', 'B', 10)
            self.cell(0, 10, 'Logo Não Encontrado', 0, 1, 'L')
...
def generate_carne_pdf_bytes(db_carne: models.Carne) -> bytes:
    try:
        ...
        pdf.ln(5)
        pdf.cell(0, 5, "Data: ___/___/_____", 0, 0, 'L')
        ...
        return bytes(pdf_byte_data) if isinstance(pdf_byte_data, bytearray) else pdf_byte_data
