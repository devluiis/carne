"""Add juros_multa_anterior_aplicada to parcela

Revision ID: f9972ed9c685
Revises: 
Create Date: 2025-06-01 20:12:56.095912
...
def upgrade() -> None:
    ...
    # ESTA É A ÚNICA LINHA (ou blocos de add_column/create_table/create_index)
    # que você quer que seja executada no upgrade para adicionar a nova coluna.
    # Se ela não foi gerada automaticamente, você pode adicioná-la manualmente:
    op.add_column('parcela', sa.Column('juros_multa_anterior_aplicada', sa.DECIMAL(10, 2), server_default='0.00', nullable=False))
...
def downgrade() -> None:
    ...
    sa.UniqueConstraint('cpf_cnpj', name='cliente_cpf_cnpj_key', postgresql_include=[], postgresql_nulls_not_distinct=False),
    postgresql_ignore_search_path=False
    )
    ...
    sa.Column('id_parcela', sa.INTEGER(), server_default=sa.text("nextval('parcela_id_parcela_seq'::regclass)"), autoincrement=True, nullable=False),
    ...
    sa.ForeignKeyConstraint(['id_carne'], ['carne.id_carne'], name='parcela_id_carne_fkey'),
    sa.PrimaryKeyConstraint('id_parcela', name='parcela_pkey'),
    sa.UniqueConstraint('id_carne', 'numero_parcela', name='parcela_id_carne_numero_parcela_key', postgresql_include=[], postgresql_nulls_not_distinct=False),
    postgresql_ignore_search_path=False
    )
